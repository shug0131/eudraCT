...
%let PATH=V:\STATISTICS\NON STUDY FOLDER\Academic Research\Eudract Tool\testing\SAS;
proc printto log="&PATH\output\log.txt";
run;
...
proc import file="&PATH\..\data\events.csv" out=ae dbms=csv replace; 
run;
/* EDIT completed. No further changes needed below this line. */


libname saswork "&PATH/SAS Eudract Tools";
...
ods pdf file="&PATH/output/Safety Data.pdf";
...
libname simple0 xml "&PATH\output\simple0.xml" ;
...
proc xsl in="&PATH\output\simple0.xml" out="&PATH\output\simple.xml" xsl="&PATH\SAS Eudract Tools\sas_xml_renaming.xslt";run;

/*use xslt to transform into the format needed by EudraCT */
proc xsl in="&PATH\output\simple.xml" out="&PATH\output\table_eudract.xml" xsl="&PATH\SAS Eudract Tools\simpleToEudraCT.xslt";run;
