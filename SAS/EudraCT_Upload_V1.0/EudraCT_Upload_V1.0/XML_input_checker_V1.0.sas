...
%let dir=P:\CTRU\Projects\Cancer\Palliative care\BETTER-B\Feasibility\Stats\Final;

/* Defining program name */
%let progname=XML_input_checker;
...
libname m "P:\CTRU\Projects\Cancer\Palliative care\BETTER-B\Feasibility\Stats\Final\SASout\Eudract";
...
/*********************************************************************/
/* Check the System organ class terms and version number are correct */
/*********************************************************************/
...
/* Create datasets of variable names variable types, length and format. */

proc contents data=EudractGrpsformat out=cont1(keep = name type length format label) noprint;
run;

proc contents data=EudractGrps out=cont2(keep = name type length format label) noprint;
run;
...
proc contents data=EudractAEformat out=cont5(keep = name type length format label) noprint;
run;

proc contents data=EudractAE out=cont6(keep = name type length format label) noprint;
run;
...
proc compare listvar
  ...
  compare=cont6 error ;
run;
/* If errors exist look at the report to see why or open up the two datasets cont5 and cont6 to compare where structures are different cont5 is the correct format. */

/*****************************************************/
/* Check to see if Eudractgrps has no missing values */
/*****************************************************/
...
/****************************************************/
/* Check to see if EudractSAE has no missing values */
/****************************************************/

data _null_;
   set EudractSAE;
   if missing(idn) then put "ERR" "OR: variable IDN has missing values in the EudractSAE dataset";
   if missing(soc) then put "ERR" "OR: variable SOC has missing values in the EudractSAE dataset";
   if missing(term) then put "ERR" "OR: variable TERM has missing values in the EudractSAE dataset";
   if missing(Asstype) then put "ERR" "OR: variable ASSTYPE has missing values in the EudractSAE dataset";
   if missing(patsn) then put "ERR" "OR: variable PATSN has missing values in the EudractSAE dataset";
   if missing(occur) then put "ERR" "OR: variable OCCUR has missing values in the EudractSAE dataset";
   if missing(occurtrt) then put "ERR" "OR: variable OCCURTRT has missing values in the EudractSAE dataset";
   if missing(death) then put "ERR" "OR: variable DEATH has missing values in the EudractSAE dataset";
   if missing(deathtrt) then put "ERR" "OR: variable DEATHTRT has missing values in the EudractSAE dataset";
run; 

/***************************************************/
/* Check to see if EudractAE has no missing values */
/***************************************************/

data _null_;
   set EudractAE;
   if missing(idn) then put "ERR" "OR: variable IDN has missing values in the EudractAE dataset";
   if missing(soc) then put "ERR" "OR: variable SOC has missing values in the EudractAE dataset";
   if missing(term) then put "ERR" "OR: variable TERM has missing values in the EudractAE dataset";
   if missing(Asstype) then put "ERR" "OR: variable ASSTYPE has missing values in the EudractAE dataset";
   if missing(patsn) then put "ERR" "OR: variable PATSN has missing values in the EudractAE dataset";
   if missing(occur) then put "ERR" "OR: variable OCCUR has missing values in the EudractAE dataset";
run; 

/*******************************************************/
/* Check to see if EudractSAE has matched SOC properly */
/*******************************************************/
...
/******************************************************/
/* Check to see if EudractAE has matched SOC properly */
/******************************************************/
...
/* Code to count number of observations in AEs dataset to make sure errors don't fire if AEs dataset is empty. */ 
proc sql noprint;
...
/***********************************************************************************************************************/
/* Check to see that there are the correct number of entries for each term & that assessment type is correctly defined */
/***********************************************************************************************************************/

data SAE_AE;
	set EudractSAE(in=a) EudractAE(in=b);
    if a then dset='EudractSAE';
	if b then dset='EudractAE';

	/* Add a check for assessment type as it needs to be 1 or 2. */
	if asstype not in (1,2) then put "ERR" "OR: Asstype in dataset=" dset +(-1)" has entries that are not 1 or 2"; 
run;
